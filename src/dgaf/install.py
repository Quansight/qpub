        ...
        if develop:
            if pip:
                ...
                if backend == "flit_core":
                    ...
                    assert not doit.tools.LongRunning("pip install -e.").execute()
        else:
            if pip:
                assert not doit.tools.CmdAction("pip install .").execute()
            elif PYPROJECT_TOML.exists():
                backend = build_backend()
                if backend == "flit_core":
                    needs("flit")
                    ...
                    assert not doit.tools.LongRunning("pip install .").execute()
